...
export class ClimbRate2Question extends QuestionBase {
  ...
  createQuestion(): void {
    ...
    this.climbToFt = this.climbFromFt + Math.floor(Math.random() * 50 + 2) * 200;
    this.timeMinutes = Math.floor(Math.random()*7)*2 + 2;
    this.climbRateFpm = (this.climbToFt - this.climbFromFt) / this.timeMinutes;
  }

  getQuestionText(): string {
    return `Men klimt in ${this.timeMinutes} minuten van ${this.climbFromFt} ft naar ${this.climbToFt} ft. Wat is de Rate of Climbg in ft/min?`;
  }
